...
# ARROW SETTINGS (Updated to gap=2, length=8)
    ...
    if st.session_state.current_index < len(matches):
        ...
        with col_viz:
            ...
            if coords.size > 0:
                ...
                axes[3].plot(traces_array[..., trace_idx-1], color='white', linewidth=1.2)
        ...
        with col_ctrl:
            ...
            # Removed the "Match X of Y" text
            st.markdown(f"<p style='text-align: center; color: gray;'>ROI: {roi_id}</p>", unsafe_allow_html=True)
            ...
            if st.session_state.decision == "Yes":
                st.success(f"Confirmed as {query}")
            elif st.session_state.decision == "No":
                st.error("Rejected")
            else:
                st.warning("Marked as Uncertain")

            st.divider()
            
            # --- CONFIDENCE (Now read from matches) ---
            st.markdown("**Confidence Level**")
            # Pull confidence from dataframe, handle NaNs, and convert to string for slider
            if pd.notna(row['Confidence']):
                default_conf = str(int(row['Confidence']))
            else:
                default_conf = "3" # Fallback if value is missing in CSV
            
            confidence = st.select_slider(
                "Conf", 
                options=["1", "2", "3", "4", "5"], 
                value=default_conf, 
                key=f"c_{roi_id}", # Dynamic key forces refresh per ROI
                label_visibility="collapsed"
            )

            # Labeling
            st.markdown("**Confirmed Neuron ID**")
            default_label = row['Neuron Class'] if pd.notna(row['Neuron Class']) else query
            final_label = st.text_input("Label", value=default_label if st.session_state.decision != "No" else "", key=f"l_{roi_id}", label_visibility="collapsed")

            # Alternatives & Notes
            st.markdown("**Alternatives / Notes**")
            alt_1 = st.text_input("Alt ID 1", key=f"a1_{roi_id}", placeholder="Alt 1...")
            notes = st.text_input("Notes", key=f"n_{roi_id}", placeholder="Optional notes...")
